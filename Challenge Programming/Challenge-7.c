...
int main(void)
{
	...
	for (i = 0; i < 100; i++)
	{
		fp[i] = malloc(sizeof(Info));
		fgets(fp[i]->name, sizeof(fp[i]->name), call);
		if (fgets(fp[i]->tell_num, sizeof(fp[i]->tell_num), call) != NULL)
		{
			count++;
		}
	}
	...
	while(1)
	{
		...
		case 5:
		{
			...
			for (i = 0; i < count; i++)
			{
				fputs(fp[i]->name, save);
				fputs(fp[i]->tell_num, save);
			}
			for (i = 0; i < count; i++)
			{
				if (fp[i] != NULL)
